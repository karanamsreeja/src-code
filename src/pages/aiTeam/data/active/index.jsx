...
import { axiosGet, axiosPut, fileupload } from 'helpers/axios';
...
const FilterList = [
	...
	{ title: 'Processing began on', id: 'date' },
];
...
const MyPickup = () => {
	...
	useEffect(() => {
		async function getData() {
			const res = await axiosGet('/ai/samples', { status: 'active' }, setloading);

			if (res.status === 200) {
				const arr = res.data.kits.map((e) => ({
					id: e._id,
					fullname: e.patient_id.firstname + ' ' + e.patient_id.lastname,
					date: OrderDateTimeFormatter(e.timestamps?.data_processing_started),
				}));
				setState(arr);
				setfilterState(arr);
			}
	...
	const handleUploadReport = async (kitId, event) => {
		console.log(kitId, event);
		...
		const inputType = event.target.files[0].type.split('/').pop();
		if (!(inputType === 'pdf')) {
			alert('File formate not suported.');
			return;
		}
		...
		const res = await fileupload('/ai/sample/report-uploaded', formData, setloading);
					...
					<Table sx={{ minWidth: 650, whiteSpace: 'nowrap' }} aria-label='simple table'>
						<TableHead>
							<TableRow>
								...
								<TableCell align='left'>Processing began on</TableCell>
									...
									<TableCell align='right'>
										<label htmlFor={e.id}>
											<input
												id={e.id}
												type='file'
												onChange={(event) => handleUploadReport(e.id, event)}
												style={{ display: 'none' }}
