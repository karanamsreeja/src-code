...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			const res = await axiosGet('/ai/sample/details', { kitId: id }, setloading);

			if (res.status === 200) {
				let details = {};

				details.began_on = OrderDateTimeFormatter(res.data.kit.timestamps.data_processing_started);
				details.recieved_on = OrderDateTimeFormatter(res.data.kit.timestamps.data_uploaded_on, { month: 'long' });
				...
				details.blood_type = res.data.kit.patient_id.blood_type;
	...
	const handleDataUploaded = async () => {
		...
		const res = await axiosPut('/ai/sample/report-uploaded', { kitId: id }, setloading);
		...
		<>
			...
			<Container className='container-x'>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton aria-label='delete' sx={{ transform: 'translateX(-16px)' }} component={Link} to='/ai-team/data/active'>
				...
				<div className={classes.box}>
					<div style={{ padding: '15px 32px', background: '#25468A' }}>
						<Typography variant='caption' letterSpacing='1px' lineHeight='20px' fontWeight='bold' color='white' textTransform='uppercase'>
							Processing began on {state.began_on}
						</Typography>
					</div>
					<Stack direction='row' className={classes.stack}>
						<div>
							...
							>
								data Recieved on: {state.recieved_on}
							</Typography>
						</div>
						<div>
							<Button
