...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			...
			if (res.status === 200) {
				...
				details.uploaded_on = OrderDateTimeFormatter(res.data.kit.timestamps.sample_approved_on, { month: 'long' });
	...
	const handleMoveInProgress = async () => {
		setloading({ message: 'Processing...' });
		const res = await axiosPut('/ai/sample/data-processing', { kitId: id }, setloading);
		...
		<>
			...
			<Container className='container-x'>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton aria-label='delete' sx={{ transform: 'translateX(-16px)' }} component={Link} to='ai-team/data/received'>
				...
				<div className={classes.box}>
					<Stack direction='row' className={classes.stack}>
						<div>
							...
							>
								Data uploaded on: {state.uploaded_on}
							</Typography>
						</div>
						<div>
							<Button
