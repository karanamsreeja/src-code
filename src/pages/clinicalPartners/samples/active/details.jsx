...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			...
			if (res.status === 200) {
				...
				details.began_on = OrderDateTimeFormatter(res.data.kit.timestamps.sample_approved_on);
				details.recieved_on = OrderDateTimeFormatter(res.data.kit.timestamps.pickup_completed_on, { month: 'long' });
	...
	const handleDataUploaded = async () => {
		...
		const res = await axiosPut('/clinical/sample/data-uploaded', { kitId: id }, setloading);
		...
		<>
			...
			<Container className='container-x'>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton aria-label='delete' sx={{ transform: 'translateX(-16px)' }} component={Link} to='/clinical-partners/samples/active'>
				...
				<div className={classes.box}>
					<div style={{ padding: '15px 32px', background: '#25468A' }}>
						<Typography variant='caption' letterSpacing='1px' lineHeight='20px' fontWeight='bold' color='white' textTransform='uppercase'>
							Sampling began on {state.began_on}
						</Typography>
					</div>
