...
function PickupsScheduledList({ handleViewOrderDetails }) {
	...
	useEffect(() => {
		async function getData() {
			...
			const res = await axiosGet('/pickups', {}, setloading);

			if (res.status === 200) {
				setList(
					res.data.scheduled_on.map((e) => {
						let obj = { id: e._id };
			...
			<div style={{ marginTop: '22px' }}>
				{list?.map((e) => (
					<Grid container className={classes.listItem} key={e.id}>
						<Grid item sx={{ minWidth: '150px', borderRight: '1px solid #E0E0E0', pr: '5px' }}>
							<Typography
								variant='caption'
								fontSize='10px'
								letterSpacing='1px'
						...
						<Grid container item xs sx={{ px: '4px' }} justifyContent='center'>
							<Stepper alternativeLabel activeStep={e.status} connector={<QontoConnector />} sx={{ width: '100%', maxWidth: '382px' }}>
								{steps.map((label) => (
									<Step key={label}>
										<StepLabel
...
export default PickupsScheduledList;
