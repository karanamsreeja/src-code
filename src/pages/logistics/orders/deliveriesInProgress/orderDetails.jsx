...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			...
			if (res.status === 200) {
				...
				product_details.assignTo = res.data.order.dispatch.dispatched_by?.fullname;
	...
	const handleOrderDelivered = async () => {
		...
		await axiosPut('/logistics/orders/delivered', { order_id: id }, setloading);
	};
			...
			<Container className={classes.container}>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton
						...
						component={Link}
						to='/logistics/orders/deliveries-in-progress'
					>
