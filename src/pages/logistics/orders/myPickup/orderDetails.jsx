...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			...
			if (res.status === 200) {
				...
				product_details.recive_on = OrderDateTimeFormatter(res.data.order.dispatch.initiated_on);
	...
	const handleOrderPickedUp = async () => {
		...
		await axiosPut('/logistics/orders/collected', { order_id: id }, setloading);
	};
		...
		<>
			...
			<Container className={classes.container}>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton aria-label='delete' sx={{ transform: 'translateX(-16px)' }} component={Link} to='/logistics/orders/my-pickup'>
