...
const DeliveriesInProgress = () => {
	...
	useEffect(() => {
		async function getData() {
			const res = await axiosGet('/logistics/pickups/in-progress', {}, setloading);

			if (res.status === 200) {
				const arr = res.data.pickup.map((e) => ({
					id: e._id,
					fullname: e.pickedup_by.fullname,
					order_on: OrderDateTimeFormatter(e.collected_on),
				}));
				setState(arr);
				setfilterState(arr);
			}
	...
	const handleOrderDelivered = async (order_id) => {
		...
		const res = await axiosPut('/logistics/pickups/delivered', { order_id }, setloading);
