...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			...
			if (res.status === 200) {
				...
				product_details.delivered_on = OrderDateTimeFormatter(res.data.pickup.delivered_on);
				product_details.pickup_on = OrderDateTimeFormatter(res.data.pickup.collected_on);
				...
				navigate('/logistics/pickups/deliveries-in-progress', { replace: true });
			}
		}
	...
	const handleOrderDelivered = async () => {
		...
		const res = await axiosPut('/logistics/pickups/delivered', { order_id: id }, setloading);
			...
			<Container className={classes.container}>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton
						...
						component={Link}
						to='/logistics/pickups/deliveries-in-progress'
					>
				...
				<div className={classes.box}>
					<div style={{ padding: '15px 32px', background: '#25468A' }}>
						<Typography variant='caption' letterSpacing='1px' lineHeight='20px' fontWeight='bold' color='white' textTransform='uppercase'>
							Ordered picked up on {state.pickup_on}
						</Typography>
					</div>
