...
function OrderDetails() {
	...
	useEffect(() => {
		async function getData() {
			const res = await axiosGet('/admin/sample/details', { kitId: id }, setloading);

			if (res.status === 200) {
				let details = {};

				details.estimated_date = '';
				details.recieved_on = OrderDateTimeFormatter(res.data.kit.timestamps.pickup_completed_on);
		...
		<>
			...
			<Container className='container-x'>
				<Stack direction='row' sx={{ mb: '30px' }}>
					<IconButton aria-label='delete' sx={{ transform: 'translateX(-16px)' }} component={Link} to='/admin/orders/sequencing-status'>
				...
				<div className={classes.box}>
					...
					<Stack direction='row' className={classes.stack}>
						...
						<div>
							<Button variant='contained' size='small' color='darkYellow' sx={{ px: '25px' }}>
